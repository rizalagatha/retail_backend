...
const getNextNumber = async (req, res) => {
    try {
        ...
        const nextNumber = await barcodeFormService.getNextBarcodeNumber(cabang, tanggal);
        res.json({ nextNumber });
    } catch (error) {
...
const searchProducts = async (req, res) => {
    try {
        ...
        if (!gudang || !category) {
            return res.status(400).json({ message: 'Parameter gudang dan kategori diperlukan.' });
        }
...
const searchMaster = async (req, res) => {
    try {
        ...
        const result = await barcodeFormService.searchMaster(term, Number(page), Number(itemsPerPage));
        res.json(result);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
