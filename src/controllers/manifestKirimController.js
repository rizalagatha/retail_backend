...
const getAvailableInvKpr = async (req, res) => {
    try {
        const { gudang, search } = req.query;
        const data = await manifestKirimService.getAvailableInvKpr(
            gudang,
            search,
        );
        res.json(data);
    ...
    try {
        ...
        if (!isNew && targetNomor !== "BARU") {
            try {
                ...
                console.warn(
                    "Gagal snapshot oldData Manifest Kirim:",
                    e.message,
                );
            }
        }
...
const remove = async (req, res) => {
    try {
        ...
        try {
            ...
            console.warn(
                "Gagal snapshot oldData remove Manifest Kirim:",
                e.message,
            );
        }
...
const updateStatus = async (req, res) => {
    try {
        ...
        if (!status) {
            return res
                .status(400)
                .json({ message: "Field 'status' wajib diisi." });
        }

        const result = await manifestKirimService.updateStatus(
            nomor,
            status,
            user,
        );
