...
const cancelReceipt = async (nomorKirim, user) => {
    ...
    try {
        ...
        const [headerRows] = await connection.query('SELECT msk_noterima, msk_kecab FROM tmsk_hdr WHERE msk_nomor = ?', [nomorKirim]);
        ...
        if (header.msk_kecab !== user.cabang) throw new Error('Anda tidak berhak membatalkan penerimaan milik cabang lain.');

        const [terimaRows] = await connection.query('SELECT mst_closing FROM tmst_hdr WHERE mst_nomor = ?', [nomorTerima]);
        ...
        await connection.query('DELETE FROM tmst_hdr WHERE mst_nomor = ?', [nomorTerima]);
        // Kosongkan referensi di header pengiriman
        await connection.query('UPDATE tmsk_hdr SET msk_noterima = "" WHERE msk_nomor = ?', [nomorKirim]);
