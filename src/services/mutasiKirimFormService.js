    ...
    try {
        ...
        if (isNew) {
            ...
            const headerInsertQuery = `
                ...
                VALUES (?, ?, ?, ?, ?, NOW());
            `;
            await connection.query(headerInsertQuery, [nomorDokumen, header.tanggal, header.storeTujuanKode, header.keterangan, user.kode]);
        } else {
            const headerUpdateQuery = `
                UPDATE tmsk_hdr SET msk_tanggal = ?, msk_kecab = ?, msk_ket = ?, user_modified = ?, date_modified = NOW()
                WHERE msk_nomor = ?;
            `;
            await connection.query(headerUpdateQuery, [header.tanggal, header.storeTujuanKode, header.keterangan, user.kode, nomorDokumen]);
        }

        await connection.query('DELETE FROM tmsk_dtl WHERE mskd_nomor = ?', [nomorDokumen]);

        if (items.length > 0) {
            const itemInsertQuery = `
                INSERT INTO tmsk_dtl (mskd_nomor, mskd_kode, mskd_ukuran, mskd_jumlah) VALUES ?;
            `;
            const itemValues = items.map(item => [nomorDokumen, item.kode, item.ukuran, item.jumlah]);
            await connection.query(itemInsertQuery, [itemValues]);
        }

        await connection.commit();
        return { message: `Data berhasil disimpan dengan nomor ${nomorDokumen}`, nomor: nomorDokumen };
    } catch (error) {
...
const getPrintData = async (nomor) => {
    ...
    const header = {
        ...
        perush_alamat: `${rows[0].gdg_inv_alamat || ''}, ${rows[0].gdg_inv_kota || ''}`, // Gabungkan alamat & kota
