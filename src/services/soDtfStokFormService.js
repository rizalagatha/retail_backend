...
const saveData = async (nomor, data, user) => {
  ...
  try {
    ...
    if (!isEdit) {
      // Logika getmaxnomor
      const prefix = `${user.cabang}.${header.jenisOrderKode}.${format(
        new Date(header.tanggal),
        "yyMM"
      )}`;
      const [maxRows] = await connection.query(
        `SELECT IFNULL(MAX(CAST(RIGHT(sd_nomor, 4) AS UNSIGNED)), 0) as maxNum FROM tsodtf_hdr WHERE LEFT(sd_nomor, ${prefix.length}) = ?`,
        [prefix]
      );
