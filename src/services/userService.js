...
const getAllUsers = async (searchTerm = '') => {
    let query = 'SELECT user_kode as kode, user_nama as nama, user_cab as cabang FROM tuser';
    const params = [];
...
const getAllBranches = async () => {
    const [rows] = await pool.query('SELECT gdg_kode FROM tgudang ORDER BY gdg_kode');
    return rows;
};
...
const getUserDetails = async (kode, cabang) => {
    // 1. Ambil data user
    const [userRows] = await pool.query('SELECT * FROM tuser WHERE user_kode = ? AND user_cab = ?', [kode, cabang]);
    ...
    const [menuRows] = await pool.query('SELECT men_id, men_nama, men_keterangan FROM tmenu WHERE men_modul=1 ORDER BY men_id');
    
    // 3. Ambil hak akses spesifik user ini
    const [hakRows] = await pool.query('SELECT * FROM thakuser WHERE hak_user_kode = ? AND hak_cab = ?', [kode, cabang]);
...
const saveUser = async (userData) => {
    ...
    try {
        ...
        if (isNewUser) {
            ...
            // Hanya update nama, password tidak diupdate di sini kecuali diminta
            await connection.query(
        ...
        await connection.query('DELETE FROM thakuser WHERE hak_user_kode = ? AND hak_cab = ?', [kode, cabang]);

        // Langkah 3: Insert hak akses yang baru
        for (const p of permissions) {
            if (p.view) { // Hanya insert jika ada hak akses
                await connection.query(
                    `INSERT INTO thakuser (hak_user_kode, hak_cab, hak_men_id, hak_men_view, hak_men_insert, hak_men_edit, hak_men_delete) 
                     VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [kode, cabang, p.id, p.view ? 'Y' : 'N', p.insert ? 'Y' : 'N', p.edit ? 'Y' : 'N', p.delete ? 'Y' : 'N']
                );
            }
        }
...
const deleteUser = async (kode, cabang) => {
    ...
    try {
        await connection.beginTransaction();
        await connection.query('DELETE FROM thakuser WHERE hak_user_kode = ? AND hak_cab = ?', [kode, cabang]);
        await connection.query('DELETE FROM tuser WHERE user_kode = ? AND user_cab = ?', [kode, cabang]);
        await connection.commit();
...
const getAllMenus = async () => {
    const [rows] = await pool.query('SELECT men_id, men_nama, men_keterangan FROM tmenu WHERE men_modul=1 ORDER BY men_id');
    return rows;
};
