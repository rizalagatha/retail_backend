...
const getList = async (filters, user) => {
  const { startDate, endDate, search } = filters; // BARU: ambil search
  ...
  // [BARU] Filter search: kode atau nama gabungan
  if (search) {
    query += `
          AND (
            a.brg_kode LIKE ?
  ...
  query += ` ORDER BY a.brg_jeniskaos, a.brg_tipe, a.brg_lengan, a.brg_jeniskain, a.brg_warna`;
...
const getExportDetails = async (filters, user) => {
  const { startDate, endDate, search } = filters; // BARU
  const inclusiveEndDate = format(addDays(parseISO(endDate), 1), "yyyy-MM-dd");
  ...
  query += ` ORDER BY b.brgd_kode, b.brgd_ukuran`;
