...
const getList = async (filters) => {
    ...
    // Query ini mereplikasi logika status dari Delphi
    const query = `
        SELECT 
...
const getDetails = async (nomor) => {
    const query = `
        SELECT 
            ...
            IFNULL((SELECT SUM(dd.mid_jumlah) FROM tmutasiin_dtl dd JOIN tmutasiin_hdr hh ON hh.mi_nomor = dd.mid_nomor WHERE hh.mi_mo_nomor = d.mod_nomor AND dd.mid_kode = d.mod_kode AND dd.mid_ukuran = d.mod_ukuran), 0) AS QtyIn
