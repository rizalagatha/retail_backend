...
/**
 * Mengambil data mentah penjualan untuk tab Data Mentah (lazy load di frontend).
 */
...
/**
 * Mengambil data teragregasi untuk PivotTable.js.
 * Agregasi per Nama Barang + Store + Bulan agar tidak crash di browser.
 * Tanpa Nomor/Customer individual — terlalu granular untuk pivot.
 */
  ...
  const query = `
    SELECT
        ...
        SUM(
          ...
          - (h.inv_disc / (
              SELECT COUNT(*) FROM tinv_dtl i
              WHERE i.invd_inv_nomor = h.inv_nomor
            ))
        ) AS Nominal
    FROM tinv_dtl d
    INNER JOIN tinv_hdr h ON d.invd_inv_nomor = h.inv_nomor
    ...
    WHERE h.inv_sts_pro = 0
      ...
      ${branchFilter}
    GROUP BY
        Store, NamaStore, Tahun, Bulan, Nama, KtgProduk, Ukuran
    ORDER BY
        Store, Tahun, Bulan, Nama, Ukuran;
  `;
