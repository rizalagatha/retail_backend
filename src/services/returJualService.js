...
const remove = async (nomor, user) => {
    ...
    try {
        ...
        if (headerRows.length === 0) throw new Error('Dokumen retur tidak ditemukan.');
        const header = headerRows[0];

        if (header.diBayarkan > 0) throw new Error('Retur ini sudah dilink ke piutang, tidak bisa dihapus.');
        if (header.closing === 'Y') throw new Error('Sudah Closing, tidak bisa dihapus.');
        if (user.cabang !== 'KDC' && header.cabang !== user.cabang) throw new Error('Anda tidak berhak menghapus data cabang lain.');
        ...
        if (header.rj_idrec) {
            await connection.query('DELETE FROM tpiutang_dtl WHERE pd_sd_angsur = ?', [header.rj_idrec]);
        }
