...
const getDasborData = async (filters) => {
    ...
    const [kuotaRows] = await pool.query('SELECT dq_kuota FROM retail.tdtf_kuota WHERE dq_cab = ?', [cabang]);
    ...
    const totalsQuery = `
        ...
        GROUP BY tgl
    `;
    const [totalRows] = await pool.query(totalsQuery, [cabang, startDate, endDate]);
