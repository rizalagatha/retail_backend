...
const getNextPotonganNumber = async (gdgKode, date) => {
    ...
    const yearMonth = String(d.getFullYear()).substring(2) + String(d.getMonth() + 1).padStart(2, '0'); // YYMM
...
const getDetails = async (nomor) => {
    ...
    const [jurnalCheck] = await pool.query('SELECT jur_nomor FROM finance.tjurnal WHERE jur_nomor = ? LIMIT 1', [nomor]);
    ...
    try {
        ...
        if (!isEdit || !ptNomor) {
            ...
            const insertHdrSql = `
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
            `;
            await connection.query(insertHdrSql, [ptNomor, ptCusKode, ptTanggal, ptAkun, ptNominal, cabKode, userKode]);
        } else {
            // UPDATE (Ubah)
            const updateHdrSql = `
                ...
                WHERE pt_nomor = ?
            `;
            await connection.query(updateHdrSql, [ptTanggal, ptNominal, ptAkun, userKode, ptNomor]);

            // Hapus Detail Lama sebelum insert yang baru
            await connection.query('DELETE FROM tpotongan_dtl WHERE ptd_nomor = ?', [ptNomor]);
            // Hapus tpiutang_dtl yang merujuk pada potongan ini (pd_ket = nomor potongan)
            await connection.query('DELETE FROM tpiutang_dtl WHERE pd_ket = ?', [ptNomor]);
        }
        ...
        for (const detail of details) {
            ...
            if (detail.invoice && bayar > 0) {
                ...
                const angsuran = detail.angsur || 
                    `${cabKode}POT${new Date().toISOString().replace(/\D/g, '').slice(2, 17)}`;
                ...
                const insertDtlSql = `
                    ...
                    VALUES (?, ?, ?, ?, ?)
                `;
                await connection.query(insertDtlSql, [ptNomor, detail.tglbayar, detail.invoice, bayar, angsuran]);
                ...
                await connection.query(insertPiutangDtlSql, [pdPhNomor, detail.tglbayar, bayar, ptNomor, angsuran]);
            }
        }
...
const remove = async (nomor, user) => {
    ...
    try {
        ...
        if (potongan.user_cab !== user.cabang && user.cabang !== 'KDC') {
             throw new Error(`Anda tidak berhak menghapus data milik cabang ${potongan.user_cab}.`);
        }

        // Cek Jurnal
        const [jurnalCheck] = await connection.query('SELECT jur_nomor FROM finance.tjurnal WHERE jur_nomor = ? LIMIT 1', [nomor]);
        ...
        await connection.query('DELETE FROM tpotongan_dtl WHERE ptd_nomor = ?', [nomor]);
        await connection.query('DELETE FROM tpiutang_dtl WHERE pd_ket = ?', [nomor]); // pd_ket adalah nomor potongan
        await connection.query('DELETE FROM tpotongan_hdr WHERE pt_nomor = ?', [nomor]);
