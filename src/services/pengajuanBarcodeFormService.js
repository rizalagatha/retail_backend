...
const save = async (payload, user) => {
  ...
  try {
    ...
    // === [FIX] PINDAHKAN LOGIKA STIKER KE SINI (DI LUAR IF-ELSE) ===
    // 1. Hapus stiker lama (selalu bersihkan agar tidak duplikat)
    await connection.query(
      "DELETE FROM tpengajuanbarcode_sticker WHERE pcs_nomor = ?",
      [nomorDokumen],
    );

    // 2. Insert stiker baru jika ada
    if (stickers && stickers.length > 0) {
      const stickerValues = stickers.map((s, i) => {
        const stickerTime = new Date(now.getTime() + i + 500);
        const pcs_idrec = `${user.cabang}ST${format(stickerTime, "yyyyMMddHHmmss.SSS")}`;

        return [
          pcs_idrec,
          nomorDokumen,
          s.kode, // Kode Kaos Induk
          s.kodes, // Kode Stiker
          s.ukuran,
          s.jumlah,
        ];
