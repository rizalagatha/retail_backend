...
const save = async (payload, user) => {
    ...
    try {
        ...
        const year = new Date(header.tanggalTerima).getFullYear().toString().substring(2);
        ...
        const headerInsertQuery = `
            ...
            VALUES (?, ?, ?, NOW());
        `;
        await connection.query(headerInsertQuery, [nomorTerima, header.tanggalTerima, user.kode]);

        await connection.query('UPDATE tmsk_hdr SET msk_noterima = ? WHERE msk_nomor = ?', [nomorTerima, header.nomorKirim]);

        const itemValues = items.map(item => [nomorTerima, item.kode, item.ukuran, item.jumlahTerima]);
        ...
        return { message: `Penerimaan berhasil disimpan dengan nomor ${nomorTerima}`, nomor: nomorTerima };
    } catch (error) {
