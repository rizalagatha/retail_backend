...
const loadFromInvoice = async (nomorInvoice) => {
    ...
    const header = {
        ...
        customer: { kode: rows[0].inv_cus_kode, nama: rows[0].cus_nama, alamat: rows[0].cus_alamat, kota: rows[0].cus_kota, telp: rows[0].cus_telp },
...
const save = async (payload, user) => {
    ...
    try {
        ...
        if (isNew) {
            const yearMonth = new Date(header.tanggal).toISOString().slice(2, 7).replace('-', '');
            const prefix = `${header.cabangKode}.RJ.${yearMonth}.`;
            const nomorQuery = `SELECT IFNULL(MAX(RIGHT(rj_nomor, 4)), 0) + 1 AS next_num FROM trj_hdr WHERE LEFT(rj_nomor, 11) = ?;`;
            const [nomorRows] = await connection.query(nomorQuery, [prefix]);
            nomorDokumen = `${prefix}${nomorRows[0].next_num.toString().padStart(4, '0')}`;
            
            const headerInsertQuery = `
                INSERT INTO trj_hdr (rj_nomor, rj_inv, rj_jenis, rj_tanggal, rj_ppn, rj_disc, rj_cus_kode, rj_ket, user_create, date_create) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
            `;
            // Gunakan footer.diskonRp untuk kolom rj_disc
            await connection.query(headerInsertQuery, [
                nomorDokumen, header.invoice, header.jenis, header.tanggal, 
                header.ppnPersen, footer.diskonRp, header.customer.kode, 
            ...
            await connection.query(headerUpdateQuery, [
                header.tanggal, header.ppnPersen, footer.diskonRp, 
                header.keterangan, user.kode, nomorDokumen
            ]);
        }
        
        await connection.query('DELETE FROM trj_dtl WHERE rjd_nomor = ?', [nomorDokumen]);

        if (items.length > 0) {
            const itemValues = items.map((item, index) => [nomorDokumen, item.kode, item.ukuran, item.jumlah, item.harga, item.disc, item.diskon, index + 1]);
        ...
        if (header.jenis === 'Y') {
            ...
            await connection.query(
                ...
                [piutangHeaderNomor, header.tanggal, payload.footer.grandTotal, nomorDokumen]
            );
        }

        await connection.commit();
        return { message: `Retur Jual berhasil disimpan dengan nomor ${nomorDokumen}`, nomor: nomorDokumen };
    } catch (error) {
...
const getPrintData = async (nomor) => {
    const query = `
        ...
        LEFT JOIN tgudang g ON g.gdg_kode = LEFT(h.rj_nomor, 3)
        WHERE h.rj_nomor = ?;
    `;
    ...
    const header = {
        ...
        customer: { nama: rows[0].cus_nama, alamat: rows[0].cus_alamat, kota: rows[0].cus_kota, telp: rows[0].cus_telp },
        gudang: { nama: rows[0].gdg_inv_nama, alamat: rows[0].gdg_inv_alamat, kota: rows[0].gdg_inv_kota, telp: rows[0].gdg_inv_telp },
        summary: {
            subtotal,
            diskon: diskonHeader,
            ppn: ppnRp,
    ...
    const details = rows.filter(r => r.rjd_kode).map(r => ({
        kode: r.rjd_kode,
        nama: r.nama_barang,
        ukuran: r.rjd_ukuran,
        jumlah: r.rjd_jumlah,
...
const getForEdit = async (nomorRetur) => {
    ...
    `;
    const [invoiceItems] = await pool.query(invoiceItemsQuery, [nomorInvoice, nomorRetur, nomorInvoice]);
    ...
    const returnedMap = new Map(returnedItems.map(item => [`${item.rjd_kode}-${item.rjd_ukuran}`, item.rjd_jumlah]));
    ...
    const headerData = {
        ...
        customer: { kode: returHeader.rj_cus_kode, nama: returHeader.cus_nama, alamat: returHeader.cus_alamat, kota: returHeader.cus_kota, telp: returHeader.cus_telp },
...
module.exports = { loadFromInvoice, save, lookupInvoices, findByBarcode, getPrintData, getForEdit, };