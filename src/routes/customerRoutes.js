...
router.get('/next-code', verifyToken, checkPermission(CUSTOMER_MENU_ID, 'view'), customerController.getNextCode);
router.get('/levels', verifyToken, checkPermission(CUSTOMER_MENU_ID, 'view'), customerController.getLevels);
...
router.post('/', verifyToken, checkPermission(CUSTOMER_MENU_ID, 'insert'), customerController.create); // POST ke /api/customers untuk membuat data baru
router.put('/:kode', verifyToken, checkPermission(CUSTOMER_MENU_ID, 'edit'), customerController.update); // PUT ke /api/customers/K0300197 untuk mengubah

// Rute Dinamis
// Membutuhkan hak 'delete' untuk menghapus
router.delete('/:kode', verifyToken, checkPermission(CUSTOMER_MENU_ID, 'delete'), customerController.remove);

// Membutuhkan hak 'view' untuk melihat detail satu customer
router.get('/:kode', verifyToken, checkPermission(CUSTOMER_MENU_ID, 'view'), customerController.getDetails);
